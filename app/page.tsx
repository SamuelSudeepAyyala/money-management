...
function Feature({ tab }: { tab: Tab }) { const content: Record<string, [string, string, string]> = { Budgets: ["Monthly budgets", "Set limits and understand category spending.", "Food & groceries · $205 of $400"], Loans: ["Loan center", "Track principal, interest, due dates, and payoff progress.", "Education loan · $223.67 due Sep 11"], Goals: ["Financial goals", "Turn your plans into visible progress.", "Emergency fund · 52% complete"] }; const [title, text, item] = content[tab] || [tab, "Coming soon", ""]; return <div className="feature-grid"><section className="panel empty-feature"><div className="feature-symbol">{tab === "Loans" ? "▤" : tab === "Goals" ? "◎" : "◒"}</div><h2>{title}</h2><p>{text}</p><div className="feature-placeholder">{item}</div><button className="primary-button">＋ Add {tab.slice(0, -1).toLowerCase()}</button></section><section className="panel checklist"><h2>What you can manage here</h2>{["Add and edit records", "See progress over time", "Keep everything in one view"].map(item => <div key={item}>✓ <span>{item}</span></div>)}</section></div>; }
