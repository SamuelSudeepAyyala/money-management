...
export function Feature({ tab }: { tab: Tab }) {
  ...
  const [records, setRecords] = useState(selected ? selected.seed : []); const [showForm, setShowForm] = useState(false);
  useEffect(() => { const saved = window.localStorage.getItem(storageKey); if (saved) { try { setRecords(JSON.parse(saved)); } catch { window.localStorage.removeItem(storageKey); } } }, [storageKey]);
