...
export function MoneyFlowWorkspace() {
  ...
  const content = backendFinance.error ? <div className="auth-error" role="alert">{backendFinance.error}</div> : activeTab === "Overview" ? <Overview transactions={transactions} balance={balance} onViewTransactions={() => setActiveTab("Transactions")} onNotice={notify} /> : activeTab === "Transactions" ? <Transactions transactions={transactions} exportCsv={exportCsv} remove={id => { void removeTransaction(id); }} /> : activeTab === "Accounts" ? <Accounts accounts={accounts} onSubmit={addAccount} /> : <Feature tab={activeTab} budgets={backendFinance.budgets} loans={backendFinance.loans} goals={backendFinance.goals} onAddBudget={backendFinance.addBudget} onAddLoan={backendFinance.addLoan} onAddGoal={backendFinance.addGoal} onRemoveBudget={backendFinance.removeBudget} onRemoveLoan={backendFinance.removeLoan} onRemoveGoal={backendFinance.removeGoal} onNotice={notify} />;
