...
export function MoneyFlowWorkspace() {
  ...
  return <main className="shell"><Sidebar activeTab={activeTab} onTabChange={setActiveTab} user={auth.user!} /><section className="content"><Topbar user={auth.user!} onLogout={() => { void auth.logout(); notify("You have been logged out"); }} onNotice={notify} /><PageHeading activeTab={activeTab} onAddTransaction={() => setShowAdd(true)} />{content}<footer>MoneyFlow <span>·</span> Built for a clearer relationship with money</footer></section>{showAdd && <AddTransactionModal accounts={accounts} onClose={() => setShowAdd(false)} onSubmit={addTransaction} />}{toast && <div className="toast">✓ {toast}</div>}</main>;
}
