...
export function Transactions({ transactions, compact = false, remove, edit, exportCsv, onViewAll }: { transactions: Transaction[]; compact?: boolean; remove?: (id: string) => void; edit?: (transaction: Transaction) => void; exportCsv?: () => void; onViewAll?: () => void }) {
  const groups = transactions.reduce<Record<string, Transaction[]>>((result, transaction) => { const key = transaction.date; (result[key] ||= []).push(transaction); return result; }, {});
  ...
  return <section className="panel transactions-panel"><div className="panel-heading"><div><h2>{compact ? "Recent transactions" : "All transactions"}</h2><p>{compact ? "Your latest money activity" : `${transactions.length} transactions grouped by date`}</p></div>{compact ? <button type="button" className="text-button" onClick={onViewAll}>View all →</button> : <button type="button" className="text-button" onClick={exportCsv}>Export CSV ↓</button>}</div><div className="transaction-list">{transactions.length ? rows : <div className="empty-state">No transactions yet. Add your first one.</div>}</div></section>;
}
