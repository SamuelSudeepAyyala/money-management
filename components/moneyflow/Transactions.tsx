...
export function Transactions({ transactions, compact = false, remove, edit, exportCsv, onViewAll }: { transactions: Transaction[]; compact?: boolean; remove?: (id: string) => void; edit?: (transaction: Transaction) => void; exportCsv?: () => void; onViewAll?: () => void }) {
  const [view, setView] = useState<"all" | Transaction["type"]>("all"); const [month, setMonth] = useState(compact ? "" : localDateString().slice(0, 7)); const [account, setAccount] = useState(""); const [type, setType] = useState<"all" | Transaction["type"]>("all"); const [category, setCategory] = useState("");
